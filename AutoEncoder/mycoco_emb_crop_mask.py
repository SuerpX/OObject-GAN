...
class CocoDetection(data.Dataset):
    ...
    def __init__(self, root, annFile, transform=None, target_transform=None, categories=None, batch_size=32):
...
#         pdb.set_trace()
        return(all(np.array(sizes)>10000))
...
#         if(all([np.sum(self.coco.annToMask(annJSON))>100 for annJSON in annJSONList])):
    ...
    def __getitem__(self, index):
        ...
        if len(target)>0:
            ...
            rm_obj = np.random.choice(target_objs)
            cat = coco.loadCats(rm_obj['category_id'])
        
            nm = cat[0]['name']
            #print(nm)
            npmask = coco.annToMask(rm_obj)*255 # small trick to prevent the discriminator
...
#             loc_enc = self.target_transform(loc_enc)
        ...
        mask_trans[:, rmin + ry : rmax + ry, cmin + cx : cmax + cx] = mask_trans_part.detach()
        obj_trans[:, rmin + ry : rmax + ry, cmin + cx : cmax + cx] = obj_trans_part.detach()
        
        transform_obj_mask = transforms.Compose([transforms.ToPILImage(),
                                                transforms.Resize((IMAGE_SIZE, IMAGE_SIZE)),
                                               transforms.ToTensor()
                                               ])
        #mask_trans_crop = transform_obj_mask(mask_trans_part)
        obj_trans_crop = transform_obj_mask(obj_trans_part)
        return obj, obj_trans_crop, obj_trans, mask_trans, nm#, loc_enc
